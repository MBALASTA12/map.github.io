...
// Handle Delivery button functionality
document.getElementById('delivery-button').addEventListener('click', async function () {
    const deliveryInput = document.getElementById('delivery-input').value;
    
    // Fetch suggestions and move marker
    if (deliveryInput.length > 2) {
        const suggestions = await fetchSuggestions(deliveryInput);
        if (suggestions.length > 0) {
            const bestResult = suggestions[0];
            moveDeliveryMarker(bestResult.lat, bestResult.lon); // Move to delivery location
        } else {
            alert('No suggestions found for delivery location.');
        }
    } else {
        alert('Please enter at least 3 characters to search for delivery location.');
    }
});

// Function to move the delivery marker to a new location
function moveDeliveryMarker(lat, lon) {
    // Add the delivery marker
    const deliveryMarker = L.marker([lat, lon]).addTo(map);
    map.setView([lat, lon], 13);

    // Update the delivery card with address and coordinates
...
function updateDeliveryCard(lat, lon) {
    // Fetch address details using reverse geocoding for delivery
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}`)
        .then(response => response.json())
        .then(data => {
            const address = data.display_name || 'Unknown address';
            ...
            // Navigate to the details page once delivery location is confirmed
...
// Function to navigate to details page with URL parameters
function navigateToDetailsPage() {
    ...
    const params = new URLSearchParams({
        ...
        pickupCoordinates: encodeURIComponent(pickupCoordinates),
        deliveryAddress: encodeURIComponent(deliveryAddress),
        deliveryCoordinates: encodeURIComponent(deliveryCoordinates)
    });

    // Navigate to the details page with the parameters
